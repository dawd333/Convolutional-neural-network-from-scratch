...
public class Validation {
    ...
    static final private File dir = new File("validation_resized_photos");
    ...
    public static void main(String[] args) {
        int validationSize = 368;
        double[][][][] images = new double[validationSize][64][64][3];
        int[][] labels = new int[validationSize][4];
                ...
                try {
                    ...
                    for (int i = 0; i < h; i++) {
                        for (int j = 0; j < w; j++) {
                            ...
                            int blue = (pixel) & 0xff;
                            images[index][j][i][0] = red;
                            images[index][j][i][1] = green;
                            images[index][j][i][2] = blue;
                            sumRed+=red;
                            sumGreen+=green;
                            sumBlue+=blue;
                        }
                    ...
                    for(int i=0; i<h; i++){
                        for(int j=0; j<w; j++){
                            numRed+= pow((images[index][j][i][0] - meanRed), 2);
                            numGreen+= pow((images[index][j][i][1] - meanGreen), 2);
                            numBlue+= pow((images[index][j][i][2] - meanBlue), 2);
                        }
                    }
                    ...
                    for(int i=0; i<h; i++){
                        for(int j=0; j<w; j++){
                            images[index][j][i][0] -= meanRed;
                            images[index][j][i][0] /= stdRed;
                            images[index][j][i][1] -= meanGreen;
                            images[index][j][i][1] /= stdGreen;
                            images[index][j][i][2] -= meanBlue;
                            images[index][j][i][2] /= stdBlue;
                        }
                    }
        ...
        for (int i=0; i<validationSize; i++){
            double[][][] conv1 = utility.convolution(images[i], f1, b1, convSpatialExtent, convStride, convZeroPadding);
        ...
        System.out.println("\nValidation set probability: " + totalAccuracy / validationSize * 100 + "%");
        ...
        System.out.println("\nValidation set 0/1 probability: " + totalBestAccuracy / validationSize * 100 + "%");
